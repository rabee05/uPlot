...
export default function uPlot(opts, data, then) {
	...
	function initScale(scaleKey) {
		...
		if (sc == null) {
			...
			if (scaleOpts.from != null) {
				...
				scales[scaleKey] = assign(
					{},
					scales[scaleOpts.from],
					scaleOpts,
					{ key: scaleKey }
				);
			} else {
	...
	/* 
	Instead of replacing the dataset, it works directly on the uplotInstance.data property, which holds the current data array.
	
	If a windowSize is specified, the function keeps only the most recent windowSize points.
	Older points are removed from the beginning of the array.
	The function calls uplotInstance.redraw() to update the graph without performing a full re-render.

	
	To test 

	const uplotInstance = new uPlot(options, initialData, document.body);

	// Simulate incoming real-time data every 100ms
		setInterval(() => {
    		const now = Date.now();
    		const newData = [ [now], // New timestamp [Math.random() * 100], // Random new Y-value];
    	// Append new data points while keeping the most recent 1000 points visible
    		appendData(uplotInstance, newData, 1000);

			}, 100); // Update every 100ms

		----------------------------------------
		Example of How Sliding Window Works
		Imagine you append 5 new points every second with a windowSize of 1000:
		At t=200s, the graph shows only the last 1000 points (from t=180s to t=200s).
		Older points (e.g., before t=180s) are discarded, reducing memory usage and improving performance.
	
	*/

	function appendData(uplotInstance, newData, windowSize = 1000) {
		// Loop through each series in the current dataset
		for (let i = 0; i < uplotInstance.data.length; i++) {
			// Append new data points for this series
			uplotInstance.data[i].push(...newData[i]);

			// Limit to the most recent 'windowSize' points
			if (uplotInstance.data[i].length > windowSize) {
				uplotInstance.data[i].splice(
					0,
					uplotInstance.data[i].length - windowSize
				);
			}
		}

		// Trigger a redraw to display the updated data
		uplotInstance.redraw(false, false);
	}

	function appendData2(uplotInstance, newX, newY, windowSize = 1000) {
		// Append new data points
		uplotInstance.data[0].push(...newX);
		uplotInstance.data[1].push(...newY);

		// Maintain a rolling window
		if (uplotInstance.data[0].length > windowSize) {
			uplotInstance.data[0].splice(
				0,
				uplotInstance.data[0].length - windowSize
			);
			uplotInstance.data[1].splice(
				0,
				uplotInstance.data[1].length - windowSize
			);
		}
